<?php
	...
	class StackoverflowModel extends SourceModel {

		protected $_name 	= 'stackoverflow_data';

		protected $_prefix = 'stackoverflow';

		protected $_search  = 'content,title';

		protected $_update_tweet = "Did %d things at Stack Overflow on my lifestream %s";
		...
		public function getServiceURL() {
			return 'http://stackoverflow.com/user/' . $this->getProperty( 'userid' ) . '/' . $this->getProperty( 'username' );
		}

		public function getServiceDescription() {
			return "Stack Overflow is a programming Q & A site that's free.";
		}
		...
		public function updateData() {
			$url = 'http://stackoverflow.com/feeds/user/' . $this->getProperty( 'userid' );
			...
			if ( $http_code != 200 ) {
				throw new Stuffpress_Exception( "Stack Overflow returned http status $http_code for url: $url", $http_code );
			}

			if ( ! ( $items = simplexml_load_string( $response ) ) ) {
				throw new Stuffpress_Exception( "Stack Overflow did not return any result", 0 );
			}
		...
		public function processConfigForm($form) {
			...
			if( $values['username'] != $this->getProperty( 'userid' ) ) {
